...
class Engine(object):
    ...
    def cycle(self):
        ...
        envcontroller = braubuddy.envcontroller.RelayHeaterController()
        thermometer = braubuddy.thermometer.DummyThermometer()
        thermostat = braubuddy.thermostat.SimpleRangedThermostat(25, lower_out=2, lower_in=1, upper_in=1, upper_out=2)
        ...
        for name, output in cherrypy.request.app.config['outputs'].items():
            try:
